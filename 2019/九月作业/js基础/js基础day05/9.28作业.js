...
var mon =5
 if( mon==3||mon==4||mon==5){
    console.log (mon,'月是春季')
}
else if( mon==6||mon==7||mon==8){
    console.log(mon,'月是夏季')
}
else if( mon==9||mon==10||mon==11){
    console.log(mon,'月是秋季')
}
...
// 22. 录入一个5位数,判断 该五位数是否为回文数。（判断第一位和第五位是否一样，第二位和第四位是否一样）
 ...
 var data = 1000;
 var x = data % 7;
 console.log('如果今天是星期六,那么今天是星期' + (teday + x - 7));
 ...
 // 25. 先接收用户输入的用户名，判断该用户名是否是admin,如果不是直接程序终止，如果是那么再次提示让用户输入密码,如果密码是88888,那么提示登录成功，否则提示登录失败
     var username='adin';
