...
module.exports.randomStr = () => {
    return "letao" + this.code(6) + new Date().getTime()
}
// 生成商户订单号
module.exports.getTrade_no = () => {
    return this.randomStr() + this.code(5)
}

// 生成签名算法
module.exports.createSign = (params) => {
    // let stringA = ""
    // Object.keys(params).sort().forEach((key) => {
    //     return stringA += `${key}=${params[key]} &`
    // })
    // return crypto.createHash("MD5").update(stringA).digest("hex").toUpperCase() 
    let stringA = Object.keys(params).sort().reduce((prev, next) => {
